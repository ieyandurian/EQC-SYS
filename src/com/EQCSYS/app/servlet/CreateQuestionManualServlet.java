...
public class CreateQuestionManualServlet extends HttpServlet {
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		ques.setQuestionDifficulty(request.getParameter("questionDifficulty"));
		ques.setQuestionMarks(Integer.parseInt(request.getParameter("questionMarks")));
		ques.setQuestionTopicCode(request.getParameter("questionTopicCode"));
		ques.setQuestionQuestion(request.getParameter("questionQuestion"));
		ques.setQuestionAnswer(request.getParameter("questionAnswer"));

		own.setShareAccess(request.getParameter("questionShare"));
		
		try {
			ques = QuestionBankDAO.createQuestionManual(ques);
			if (ques.isValid()) {
				...
				if (own.isValid()){
					...
					response.sendRedirect("userLecturer/managequestion.jsp");
				} else {
